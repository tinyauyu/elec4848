...
# output format
echo "$ENCRYPTION_ALG ($TYPE key encryption) using $LIBRARY benchmark [average of $NUM encryptions]" > "$ENCRYPTION_ALG"_benchmarkTemp.txt
echo "Key Length	Encryption Time(us)	Decryption Time(us)" >> "$ENCRYPTION_ALG"_benchmarkTemp.txt
...
do
	...
	for (( c=1; c<=$NUM; c++))
	do
		lastResult="$($PRG_CALL $key $msg $TYPE)"
		#echo "$lastResult"
		t="encryption"
...
mv "$ENCRYPTION_ALG"_benchmarkTemp.txt "$ENCRYPTION_ALG"_"$LIBRARY"_"$TYPE"_benchmark.txt
echo
cat "$ENCRYPTION_ALG"_"$LIBRARY"_"$TYPE"_benchmark.txt